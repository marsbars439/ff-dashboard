...
const renderMarkdown = (text = '') => {
  ...
  const createSimpleList = (items, key) => (
    <ul key={key} className="mb-2 sm:mb-3 space-y-0.5 sm:space-y-1">
      {items.map((item, i) => {
        const indentStyle = {
  ...
  const processLine = (line, index) => {
    ...
    if (line.startsWith('# ')) {
      elements.push(
        <h1 key={index} className="text-base sm:text-2xl md:text-3xl font-bold text-slate-50 mt-3 sm:mt-6 mb-1.5 sm:mb-3">
          {processInlineFormatting(line.slice(2))}
        </h1>
      );
    } else if (line.startsWith('## ')) {
      elements.push(
        <h2 key={index} className="text-sm sm:text-xl md:text-2xl font-bold text-slate-100 mt-2 sm:mt-4 mb-1 sm:mb-2">
          {processInlineFormatting(line.slice(3))}
        </h2>
      );
    } else if (line.startsWith('### ')) {
      elements.push(
        <h3 key={index} className="text-xs sm:text-lg md:text-xl font-bold text-slate-200 mt-1.5 sm:mt-3 mb-1 sm:mb-2">
      ...
      elements.push(
        <p key={index} className="text-slate-100/90 mb-1 sm:mb-1.5 text-xs sm:text-sm md:text-base">
