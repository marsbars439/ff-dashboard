...
const snapshotCloudflareHeaders = (headers = {}) => {
  ...
  const relevantHeaderNames = [
    ...
    'cf-visitor',
    'cf-worker',
    'cf-access-proxy-session-id',
    'cf-ew-via',
    'cf-ipcountry',
    'cf-access-client-id',
    'cf-access-client-secret',
    'cf-access-policy',
    'cf-access-service-auth-domain',
    'manager-email',
    'x-forwarded-for',
    'x-forwarded-host',
    'x-forwarded-proto'
  ...
  for (const headerName of relevantHeaderNames) {
    if (!(headerName in lowerCasedHeaders)) {
      snapshot[headerName] = null;
      continue;
    }

    const value = lowerCasedHeaders[headerName];

    if (headerName === 'cf-access-jwt-assertion') {
      snapshot[headerName] = redactJwtAssertion(Array.isArray(value) ? value[0] : value);
    } else {
      snapshot[headerName] = value;
    }
  }

  snapshot.__observedHeaderNames = Object.keys(headers);
...
app.get('/api/manager-auth/cloudflare', async (req, res) => {
  ...
  logCloudflareDebug('Cloudflare header snapshot for request', snapshotCloudflareHeaders(req.headers));
