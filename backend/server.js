...
// Get playoff bracket for a specific season
app.get('/api/seasons/:year/playoff-bracket', (req, res) => {
  const year = req.params.year;
  const query = 'SELECT league_id FROM league_settings WHERE year = ?';
  db.get(query, [year], async (err, row) => {
    if (err) {
    ...
    try {
      ...
      const bracket = await sleeperService.getPlayoffBracket(row.league_id, managers);
      res.json({ bracket });
    } catch (error) {
      res.status(500).json({ error: error.message });
    }
